...
// Contract-driven data: the deck reads its numbers from the deck data contract (no manual entry).
const { load, deckData } = require("./contract_to_deck");
const CONTRACT = process.env.DECK_CONTRACT || "./ba_lhr_sjc_deck_contract.json";
const D = deckData(load(CONTRACT));
...
{ const s=pres.addSlide(); s.background={color:WHITE}; header(s);
  ...
  // three figures
  const figClr=[DARK,DARK2,CYAN]; const figs=D.figs.map(([l,v],i)=>[l,v,figClr[i]]);
...
{ const s=pres.addSlide(); s.background={color:WHITE}; header(s);
  ...
  stat(s,MX,1.7,3.4,2.0,"$175,491","Median household income","#1 of the 50 largest US metros, circa double the US average",DARK);
  s.addText("Median household income by US metro ($000s)",{x:MX+3.75,y:1.62,w:7.85,h:0.3,fontSize:11,fontFace:T,color:DARK,bold:true,margin:0});
  s.addChart(pres.charts.BAR,[{name:"Income",labels:["San Jose","San Francisco","Washington DC","Seattle","New York","US average"],values:[175.5,141.0,122.0,116.0,98.0,81.6]}],{x:MX+3.6,y:1.95,w:8.0,h:1.85,barDir:"bar",chartColors:[DARK2],showValue:true,dataLabelPosition:"outEnd",dataLabelColor:INK,dataLabelFontSize:9,dataLabelFontFace:B,catAxisLabelColor:MUT,catAxisLabelFontSize:9,valAxisHidden:true,valGridLine:{style:"none"},catGridLine:{style:"none"},showLegend:false,chartArea:{fill:{color:WHITE}}});
  bullets(s,MX,3.95,11.9,2.0,["San Jose - Sunnyvale - Santa Clara is the richest major metro in the country, with median household income roughly double the US average.","The San Jose - San Francisco cluster has ranked first in the world for innovation for six consecutive years (WIPO 2025), leading on patents and venture deals per head.","The most concentrated set of global technology headquarters anywhere, led by Apple, Alphabet and NVIDIA."],12);
...
{ const s=pres.addSlide(); s.background={color:WHITE}; header(s);
  ...
  const rows=[hd]; D.seg.forEach((r,i)=>rows.push(r.map((c,j)=>cell(c,j===0?"left":"right",i))));
  rows.push(D.segTotals.p2p.map((c,j)=>cell(c,j===0?"left":"right",0,true,DARK)));
  rows.push(D.segTotals.cnxHub.map((c,j)=>cell(c,j===0?"left":"right",1)));
  rows.push(D.segTotals.cnxDest.map((c,j)=>cell(c,j===0?"left":"right",0)));
  rows.push(D.segTotals.grand.map((c,j)=>cell(c,j===0?"left":"right",1,true,DARK)));
...
{ const s=pres.addSlide(); s.background={color:WHITE}; header(s);
  ...
  const cities=D.cnxHubCities.slice(0,15);
  ...
  rows.push(D.cnxHubTotal.map((c,j)=>cell(c,(j>=4)?"right":"left",0,true,DARK)));
...
{ const s=pres.addSlide(); s.background={color:WHITE}; header(s);
  ...
  add("Passengers - Point to Point",D.rev.pax.p2p);
  add("Passengers - Connecting at London",D.rev.pax.cnxHub);
  add("Passengers - Connecting at San Jose",D.rev.pax.cnxDest);
  add("Total Passengers",D.rev.pax.total,true);
  add("Annual Capacity",D.rev.cap);
  add("Implied Load Factor",D.rev.lf,true);
  add("Revenue - Point to Point",D.rev.revenue.p2p);
  add("Revenue - Connecting at London",D.rev.revenue.cnxHub);
  add("Revenue - Connecting at San Jose",D.rev.revenue.cnxDest);
  add("Revenue - Cargo",D.rev.revenue.cargo);
  add("Revenue - Ancillary",D.rev.revenue.ancillary);
  add("Total Revenue",D.rev.revenue.total,true);
...
{ const s=pres.addSlide(); s.background={color:WHITE}; header(s);
  title(s,"Forecast Methodology","Schedule quality drives an airline's share of demand");
  s.addText("Base traffic demand is grown to maturity from Sabre MI O&D data (adjusted for non-MIDT channels), counting only the San Jose Service Area and split into business and leisure/VFR.",{x:MX,y:1.62,w:11.9,h:0.6,fontSize:12,fontFace:B,color:DARK2,margin:0,valign:"top"});
  ...
  s.addText("Point to Point",{x:MX+0.25,y:2.55,w:5.2,h:0.35,fontSize:14,fontFace:T,color:DARK,bold:true,margin:0});
  bullets(s,MX+0.25,3.0,5.2,2.7,["Sabre MI gives the historic O&D market between the catchment and London.","A compound annual growth rate grows it to the service year.","Stimulation from the new nonstop is applied from historical benchmarks.","Capture is set from frequency-share analysis and traffic-leakage estimates."],11.5);
  ...
  s.addText("Connecting Markets",{x:MX+6.2,y:2.55,w:5.2,h:0.35,fontSize:14,fontFace:T,color:DARK,bold:true,margin:0});
  bullets(s,MX+6.2,3.0,5.15,2.7,["Sabre MI gives the connecting markets over the hub, excluding double connections.","Demand grows to maturity on the same basis.","A Quality of Service Index allocates demand across competing itineraries.","QSI scores total elapsed time, connection type (online / interline / alliance) and frequency."],11.5);
  src(s,"AviaSolutions analysis"); footer(s);
}
{ const s=pres.addSlide(); s.background={color:WHITE}; header(s);
  title(s,"Revenue and Schedule Methodology","From the QSI forecast to the route P&L");
  s.addShape(pres.shapes.ROUNDED_RECTANGLE,{x:MX,y:1.7,w:5.7,h:4.15,fill:{color:LIGHT},line:{color:PANEL,width:1},rectRadius:0.04});
  s.addText("Schedule and Capacity",{x:MX+0.25,y:1.85,w:5.2,h:0.35,fontSize:14,fontFace:T,color:DARK,bold:true,margin:0});
  bullets(s,MX+0.25,2.3,5.2,3.3,["The forecast assumes a specific schedule: outbound and return timings, layover and elapsed times.","A daily (7x weekly) 214-seat Boeing 787.","Schedule quality drives the QSI competition against all existing one-stop options.","Annual capacity of 155,792 two-way seats."],11.5);
  s.addShape(pres.shapes.ROUNDED_RECTANGLE,{x:MX+5.95,y:1.7,w:5.65,h:4.15,fill:{color:LIGHT},line:{color:PANEL,width:1},rectRadius:0.04});
  s.addText("Revenue",{x:MX+6.2,y:1.85,w:5.2,h:0.35,fontSize:14,fontFace:T,color:DARK,bold:true,margin:0});
  bullets(s,MX+6.2,2.3,5.15,3.3,["Passenger growth follows the QSI forecast.","Fares come from MIDT, weighted down with a business-fare reduction and split by cabin, with spill traffic estimated.","Cargo revenue is built from belly capacity at conservative yields, benchmarked to volume.","Ancillary revenue is sourced from industry reports."],11.5);
  src(s,"AviaSolutions analysis"); footer(s);
}
