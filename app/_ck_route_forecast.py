...
def origin_catchment_poo(sabre_db, origin_airport, dest_codes, year=None, radius_km=DEMAND_RADIUS_KM,
    ...
    sql = (f"SELECT poo_city, ANY_VALUE(poo_city_name) nm, SUM(passengers) p FROM sabre "
           f"WHERE {' AND '.join(where)} GROUP BY poo_city ORDER BY p DESC LIMIT {int(top)}")
    con = _con(sabre_db)
    try:
...
def forecast(sabre_db, oag_db, week, origin, dest_codes, competing_airports, *, year=None,
             ...
             dest_airport=None, airline=None, catchment_mult=1.0, feed_cfg=None, **_ignore):
    ...
    if airline and dest_airport:
        try:
            ...
            bt, beyond_pdew = RFEED.feed_side(sabre_db, oag_db, week, competing_airports, dest_airport,
                                              year, beyond=True, airline=airline, feed_cfg=feed_cfg)
            ...
            ht, behind_pdew = RFEED.behind_feed(sabre_db, oag_db, week, [origin], [dest_airport], year,
                                                airline=airline, feed_cfg=feed_cfg)
    ...
    if annual_capacity is None:
        try:
            ...
            seats = 180
        annual_capacity = seats * freq * 52 * 2
