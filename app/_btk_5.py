...
def asif_forecast(route, oag, sabre, served_cache, wby, stimulation=1.15, radius_km=220.0, outturn_offset=1,
    ...
    r = RF.forecast(sabre, oag, asif_week, dep, dest_codes, competing, year=Y - 1, freq=freq,
                    ...
                    annual_capacity=(annual_cap or None), catchment_mult=cmult)
...
def main():
    ...
    for i, r in enumerate(routes):
        try:
            ...
            f = asif_forecast(r, a.oag, a.sabre, served_cache, wby, a.stim, a.radius_km, offset, a.lcc_cat)
            ...
            rp = f"{ratio_p2p:>7.2f}" if ratio_p2p else f"{'-':>7}"
            rs = f"{ratio:>7.2f}" if ratio else f"{'-':>7}"
            print(f"{r['dep']+'-'+r['arr']:12} {r['type']:9} {r['year']:>4} "
                  f"{graded:>10,.0f} {p2p:>9,.0f} {rp} {out:>9,.0f} {rs} {'H' if hub else ' '} {r['carrier']}")
