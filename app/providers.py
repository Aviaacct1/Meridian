...
class Itinerary:
    ...
    et_coeff: float = 0.0
    cnx_coeff: float = 0.0
    service_coeff: float = 0.0  # non-stop 1.0 / one-stop 0.20 (established QSI method)
    qsi: float = 0.0
...
class ExcelDemandProvider(DemandProvider):
    ...
    def _load_connecting(self, direction: str) -> List[ConnectingCityData]:
        ...
        for r in range(header_row_idx + 1, len(rows)):
            ...
            demand_idx = col_map.get('base total demand', 7)
            try:
                base_demand = float(row[demand_idx]) if demand_idx < len(row) and row[demand_idx] else 0
            except (ValueError, TypeError):
                base_demand = 0
