...
class ForecastAssembler:
            ...
            if seg.subsegments:
                ...
                for sub in seg.subsegments:
                    ...
                    self.log(f"  {seg.name}/{sub.name}: {sub.base_demand:,.0f}  "
                             f"{forecast:,.0f} (CR={sub.capture_rate:.0%})")
                total += seg_total
            else:
                ...
                self.log(f"  {seg.name}: {seg.base_demand:,.0f}  "
                         f"{forecast:,.0f} (CR={seg.capture_rate:.0%})")
    ...
    def assemble_connecting(self, demand_provider: DemandProvider,
        ...
        Uses expert QSI from forecast file when available (calibrated values),
        falling back to pipeline-computed QSI captures when expert data is missing.
        This is the Chat 16 CalibrationEngine integration.
        """
        ...
        total = 0.0
        matched = unmatched = 0
        used_expert = used_pipeline = 0

        for city in cities:
            # Skip "Total" summary rows
            if city.city_code.lower() in ('total', 'totals', 'sum', ''):
                continue

            pipeline_capture = qsi_captures.get(city.city_code, 0)
            expert_qsi = city.qsi_score  # From forecast file (expert-calibrated)

            # Priority: expert QSI from forecast file always takes precedence.
            # If expert_qsi is 0, that's a deliberate analyst decision to exclude
            # this city (e.g., "without India" scenarios). Don't override with pipeline.
            # Only fall back to pipeline QSI if the city has NO entry in the forecast 
            # file at all (which would mean it wasn't loaded  but since we're iterating
            # over cities from the demand provider, they all have entries).
            # Therefore: ALWAYS use expert_qsi from the forecast file.
            capture = expert_qsi if expert_qsi is not None else 0
            if capture > 0:
                used_expert += 1
            elif pipeline_capture > 0:
                used_pipeline += 1
            ...
            results.append({
                ...
                'qsi_capture': pipeline_capture,
                'original_qsi': expert_qsi,
        ...
        self.log(f"  QSI source: {used_expert} expert, {used_pipeline} pipeline")
...
def validate(config: RouteConfig, results: Dict[str, Any]) -> bool:
    ...
    if config.target_total > 0:
        ...
        print(f"  Load factor: {results['load_factor']:.1%} (target: {config.target_load_factor:.1%})")
    else:
        print(f"\n  No regression targets set (portal mode)")
...
def main():
    ...
    outpath = '/mnt/user-data/outputs/Closed_Loop_V2_BA_LHR_SJC.xlsx'
