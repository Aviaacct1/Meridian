...
class RouteConfig:
    ...
    def ke_icn_sjc(cls, project_dir: str = '/mnt/project') -> 'RouteConfig':
        ...
        Target P2P: 37,382 pax + China connecting: 46,331 = 83,713

        Key characteristics:
        - New route proposal (KE operates ICN-SFO but not ICN-SJC)
        - Sixth-freedom Asian hub carrier with massive ICN connecting
        - QSI adjustment = 1.0 (model accepted raw  new route)
        - P2P: SK visitors 58% / US residents 42%; Business dominant
        - China connecting (HKG/CAN/SZX) treated as separate P2P-like markets
        - Hub connecting at ICN/SJC requires QSI model run (not in file)
        """
        cfg = cls()
        ...
        # Validation targets  P2P + China connecting only
        # Hub connecting at ICN/SJC not populated in source file
        cfg.target_p2p = 37382
        cfg.target_cnx_home = 0
        cfg.target_cnx_dest = 0
        cfg.target_total = 83713  # P2P (37,382) + China (46,331)
...
def _ke_icn_sjc_p2p_config() -> Dict[str, Any]:
    """
    KE ICN-SJC P2P demand configuration.
    From FcstKE_ICNSJC_212Aug19.xlsm Forecast Finalised sheet.
    
    SK (Visitors 58%): Business 90%, Leisure 10%
    US (Residents 42%): Business ~80%, Leisure ~20%
    Growth: Business 7%, Leisure 24% (post-pandemic recovery compound)
    """
    return {
        'segments': [
            {
