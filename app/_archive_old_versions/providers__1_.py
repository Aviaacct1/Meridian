...
class ExcelDemandProvider(DemandProvider):
    ...
    def _load_connecting(self, direction: str) -> List[ConnectingCityData]:
        ...
        for r in range(header_row_idx + 1, len(rows)):
            ...
            if city_code.lower() in ('total', 'totals', 'sum', 'grand total'):
                continue
            ...
            # Base demand: use Indirect Demand (col 6) for connecting traffic.
            # Analyst formula: Base_Indirect * (1+growth) * QSI.
            # For cities with direct service, Indirect != Total.
